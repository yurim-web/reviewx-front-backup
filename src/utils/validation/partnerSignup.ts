...
import { validateBusinessNumber } from "@/components/partner/signup/utils/validation";
...
export function validatePartnerSignupForm(
  ...
  const emailError = validateSignupEmailField(formData.email, true);
  ...
  if (!formData.password) {
    ...
    errors.password = "8~16자 영문, 숫자, 특수문자(!@#$%^&*()-_=+) 조합으로 입력해 주세요.";
  }

  // 비밀번호 확인 검증 (빈 필드는 에러 객체에만 저장, 텍스트는 표시 안 함)
  if (!formData.passwordConfirm) {
    errors.passwordConfirm = ""; // 빈 문자열로 에러 상태만 표시 (테두리만 빨간색)
  } else if (!validatePasswordMatch(formData.password, formData.passwordConfirm)) {
    errors.passwordConfirm = "비밀번호가 일치하지 않습니다.";
  }

  // 이름 검증 (빈 필드는 에러 객체에만 저장, 텍스트는 표시 안 함)
  const nameError = validateSignupNameField(formData.name);
  if (nameError !== undefined) errors.name = nameError;

  // 휴대폰 번호 검증 (빈 필드는 에러 객체에만 저장, 텍스트는 표시 안 함)
  const phoneError = validateSignupPhoneField(formData.phone, formData.isPhoneVerified);
  ...
  if (!formData.contactPhone) {
    ...
    if (!phoneRegex.test(formData.contactPhone)) {
      errors.contactPhone = "올바른 휴대폰 번호 형식을 입력해주세요. (010-0000-0000)";
    }
  }
