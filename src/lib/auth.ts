  ...
  if (role === 'user') {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (storedAccounts) {
          ...
          const userAccount = accounts.find((a: any) => a.id === account.id || a.email === account.email);
          if (userAccount) {
            ...
            if (!nickname || nickname === userAccount.name) {
              ...
              if (nickname) {
                const accountIndex = accounts.findIndex((a: any) => a.id === userAccount.id || a.email === userAccount.email);
                if (accountIndex >= 0) {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (storedAccounts) {
          const accounts = JSON.parse(storedAccounts);
          const partnerAccount = accounts.find((a: any) => a.email === account.email);
          if (partnerAccount) {
...
function applyPostLoginSideEffects(account: UnifiedAccount, authUser: AuthUser): void {
  ...
  if (authUser.role === 'user' && typeof window !== 'undefined') {
    try {
      ...
      // 이미 계정이 있는지 확인
      const existingIndex = accounts.findIndex((a: any) => a.id === authUser.id || a.email === authUser.email);
      ...
      if (existingIndex === -1) {
        ...
        // nickname이 없거나 name과 같은 경우 기본 데이터에서 가져오기
        if (defaultData && (!existingAccount.nickname || existingAccount.nickname === existingAccount.name)) {
        ...
        if (defaultData && defaultData.channel_details) {
          ...
          defaultDetails.forEach((defaultDetail: any) => {
            const existingIndex = mergedDetails.findIndex((d: any) => d.name === defaultDetail.name);
            if (existingIndex >= 0) {
              // 기존 채널이 있으면 status가 'connected'인 경우 URL과 통계 정보도 업데이트
              if (defaultDetail.status === 'connected' && mergedDetails[existingIndex].status !== 'connected') {
                mergedDetails[existingIndex] = {
                  ...mergedDetails[existingIndex],
                  ...defaultDetail,
                };
              } else if (mergedDetails[existingIndex].status === 'connected' && defaultDetail.status === 'connected') {
                ...
                mergedDetails[existingIndex] = {
                  ...
                  daily_visits: existingDetail.daily_visits && existingDetail.daily_visits > 0 
                    ? existingDetail.daily_visits 
                    : defaultDetail.daily_visits,
                  total_visits: existingDetail.total_visits && existingDetail.total_visits > 0 
                    ? existingDetail.total_visits 
                    : defaultDetail.total_visits,
                  neighbors: existingDetail.neighbors && existingDetail.neighbors > 0 
                    ? existingDetail.neighbors 
                    : defaultDetail.neighbors,
                  followers: existingDetail.followers && existingDetail.followers > 0 
                    ? existingDetail.followers 
                    : defaultDetail.followers,
                  subscribers: existingDetail.subscribers && existingDetail.subscribers > 0 
        ...
        if (defaultData) {
          if (defaultData.daily_visits !== undefined && (!existingAccount.daily_visits || existingAccount.daily_visits === 0)) {
            updates.daily_visits = defaultData.daily_visits;
          }
          if (defaultData.total_visits !== undefined && (!existingAccount.total_visits || existingAccount.total_visits === 0)) {
            updates.total_visits = defaultData.total_visits;
          }
          if (defaultData.neighbors !== undefined && (!existingAccount.neighbors || existingAccount.neighbors === 0)) {
  ...
  if (authUser.role === 'partner' && typeof window !== 'undefined') {
    try {
      const storedAccounts = localStorage.getItem('partner_accounts');
      const accounts = storedAccounts ? JSON.parse(storedAccounts) : [];
      const accountIndex = accounts.findIndex((a: any) => a.id === authUser.id || a.email === authUser.email);
      ...
      if (accountIndex >= 0) {
        ...
        const newPartnerAccount = {
          ...
          signupDate: account.signupDate || new Date().toISOString().split('T')[0].replace(/-/g, '. '),
          isBlocked: false,
          isBanned: false,
          redirectUrl: '/partner',
        ...
        localStorage.setItem('partner_accounts', JSON.stringify(accounts));
        console.log('✅ [로그인] partner_accounts 생성 (초기 포인트:', initialPoints, '):', newPartnerAccount);
      }
    } catch (error) {
