...
export async function fetchUserNoticeList(params?: NoticeListParams): Promise<NoticeListResponse> {
  const { data } = await apiClient.get<NoticeListResponse>("/api/v1/reviewer/notices", {
    params,
  });
  return data;
}
...
export async function fetchUserNoticeDetail(boardId: number): Promise<NoticeDetailResponse> {
  const { data } = await apiClient.get<NoticeDetailResponse>(`/api/v1/reviewer/notices/${boardId}`);
  return data;
}
