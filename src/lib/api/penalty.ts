...
export const fetchUserPenalties = (reviewerId: number): Promise<PenaltyItem[]> =>
  apiClient
    .get<PenaltyItem[]>(`/reviewer/penalty?reviewer_id=${reviewerId}`)
    .then((res) => (Array.isArray(res.data) ? res.data : []));

/**
 * 유저(리뷰어) 패널티 상태 조회
 * GET /reviewer/penalty/status?reviewer_id=:id → /user_penalty_status?reviewer_id=:id
 */
export const fetchUserPenaltyStatus = (reviewerId: number): Promise<PenaltyStatusData | null> =>
  apiClient
    .get<PenaltyStatusData[]>(`/reviewer/penalty/status?reviewer_id=${reviewerId}`)
    .then((res) => (Array.isArray(res.data) && res.data.length > 0 ? res.data[0] : null));

/**
 * 파트너 패널티 내역 조회
 * GET /partner/penalty?partner_id=:id → /partner_penalties?partner_id=:id
 */
export const fetchPartnerPenalties = (partnerId: number): Promise<PenaltyItem[]> =>
  apiClient
    .get<PenaltyItem[]>(`/partner/penalty?partner_id=${partnerId}`)
    .then((res) => (Array.isArray(res.data) ? res.data : []));

/**
 * 파트너 패널티 상태 조회
 * GET /partner/penalty/status?partner_id=:id → /partner_penalty_status?partner_id=:id
 */
export const fetchPartnerPenaltyStatus = (partnerId: number): Promise<PenaltyStatusData | null> =>
  apiClient
    .get<PenaltyStatusData[]>(`/partner/penalty/status?partner_id=${partnerId}`)
    .then((res) => (Array.isArray(res.data) && res.data.length > 0 ? res.data[0] : null));
