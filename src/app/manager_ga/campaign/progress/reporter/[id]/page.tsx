/* ========================================
   📰 GA 관리자 기자단 캠페인 상세 페이지 (동적)
   ======================================== */

/**
 * 기자단 캠페인 진행 현황 상세 페이지 (GA 관리자 버전)
 *
 * 목적: GA 관리자가 진행 현황 테이블에서 특정 기자단 캠페인을 클릭했을 때
...
export default function ManagerReporterProgressDetailPage() {
  ...
  useEffect(() => {
    const load_campaign_data = async () => {
      try {
        ...
        console.error('GA 기자단 진행현황 데이터 로딩 실패:', error);
  ...
  const handle_download_applicants = () => {
    console.log('[GA 기자단] 신청자 목록 다운로드');
  };

  const handle_download_selected = () => {
    console.log('[GA 기자단] 선정자 목록 다운로드');
  };
