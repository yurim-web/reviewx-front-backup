 ...
 * PartnerDetailPage
 *
 * 목적: GA 관리자가 특정 파트너의 상세 정보를 확인합니다.
 *
 * 사용 페이지:
...
import { usePartnerDetailData } from "@/hooks/manager/ga/member/usePartnerDetailData";
...
export default function PartnerDetailPage() {
  ...
  const activity_info_items: ActivityInfoItem[] = [
    {
      label: "캠페인 진행",
      value: partner_detail ? `${partner_detail.campaign_in_progress.toLocaleString()}회` : "0회",
      on_button_click: () => set_is_campaign_history_modal_open(true),
      button_aria_label: "캠페인 진행 내역 보기",
    },
    {
      label: "캠페인 완료",
      value: partner_detail ? `${partner_detail.campaign_completed.toLocaleString()}회` : "0회",
    },
    {
      label: "패널티",
      value: partner_detail ? `${partner_detail.penalty_count.toLocaleString()}회` : "0회",
    ...
    {
      label: "보유 포인트",
      value: partner_detail ? partner_detail.current_points.toLocaleString() : "0",
    },
    {
      label: "결제 포인트",
      value: partner_detail ? partner_detail.payment_points.toLocaleString() : "0",
    },
  ];
