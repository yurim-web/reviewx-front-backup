  ...
  useEffect(() => {
    const loadCampaignData = async () => {
      try {
        ...
        // 디버깅을 위한 콘솔 로그
        console.log("구매평 캠페인 데이터 로드:", {
          campaignId,
          category: data.campaignInfo.category,
          applicantsCount: data.applicantData.applicants.length,
          selectedCount: data.applicantData.selectedApplicants.length,
          firstApplicant: data.applicantData.applicants[0],
        });
      } catch (err) {
