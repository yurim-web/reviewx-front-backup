 ...
 */
function convertStoredToReviewCampaignData(
...
export default function ReviewPage() {
  ...
  const [allCampaigns, setAllCampaigns] = useState<ReviewCampaignData[]>(reviewCampaigns);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // localStorage의 캠페인을 ReviewCampaignData 형식으로 변환
      const convertedCampaigns = storedCampaigns.map((campaign) =>
        convertStoredToReviewCampaignData(campaign)
      );

      // localStorage 내에서도 중복 ID 제거 (같은 ID가 있으면 마지막 것만 유지)
      const uniqueStoredCampaigns = new Map<string, ReviewCampaignData>();
