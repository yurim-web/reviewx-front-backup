 ...
 */
function convertStoredToDeliveryCampaignData(
...
export default function DeliveryPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!stored) {
        setAllCampaigns(deliveryCampaigns);
        return;
      }
      ...
      // localStorage의 캠페인을 DeliveryCampaignData 형식으로 변환
      const convertedCampaigns = storedCampaigns.map((campaign) =>
        convertStoredToDeliveryCampaignData(campaign)
      );

      // localStorage 내에서도 중복 ID 제거 (같은 ID가 있으면 마지막 것만 유지)
      const uniqueStoredCampaigns = new Map<string, DeliveryCampaignData>();
