...
/**
 * CampaignWithApplicants를 MissionCampaignData 형식으로 변환하는 함수
 */
function convertStoredToMissionCampaignData(
...
export default function MissionPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!stored) {
        setAllCampaigns(missionCampaigns);
        return;
      }
      ...
      // localStorage의 캠페인을 MissionCampaignData 형식으로 변환
      const convertedCampaigns = storedCampaigns.map((campaign) =>
        convertStoredToMissionCampaignData(campaign)
      );

      // localStorage 내에서도 중복 ID 제거 (같은 ID가 있으면 마지막 것만 유지)
      const uniqueStoredCampaigns = new Map<string, MissionCampaignData>();
