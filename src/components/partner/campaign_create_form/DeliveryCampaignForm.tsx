...
export default function DeliveryCampaignForm({
   ...
   *
   * 설명:
   * - 내 정보에서 설정된 휴대폰 번호를 가져옵니다.
   * - localStorage 또는 API에서 가져올 수 있습니다.
   * - TODO: 실제 API 연동 시 수정 필요
   */
  const getDefaultContactPhone = (): string => {
    if (typeof window === "undefined") return "";

    try {
      // localStorage에서 파트너 정보 가져오기 시도
      // 실제 구현 시에는 API 호출로 대체
      const savedPhone = localStorage.getItem("partner_contact_phone");
      if (savedPhone) return savedPhone;

      // 기본값 (실제로는 API에서 가져와야 함)
      return "";
    } catch {
      return "";
    }
  ...
  const isFormValid = useMemo(() => {
    ...
    const hasRequiredFields =
      ...
      validatePhone((formData.contactPhone || "").trim()) && // 문의 담당자 휴대폰 번호 (010-XXXX-XXXX 형식)
   ...
   *
   * 설명:
   * - 배송형 캠페인의 차감 포인트는 {추가 지급 포인트 × 모집 인원}입니다.
   */
  ...
  const STORAGE_KEY = "temp_delivery_campaign";
  ...
  const handleSaveConfirm = () => {
    try {
      ...
      const dataToSave = { ...formData };
      // File 객체는 저장하지 않음 (이미지는 별도 처리 필요)
      localStorage.setItem(STORAGE_KEY, JSON.stringify(dataToSave));
   ...
   * - hydration mismatch를 방지하기 위해 클라이언트에서만 값을 계산합니다.
   * - 서버에서는 기본값(true)을 사용하고, 클라이언트에서 마운트된 후 실제 값을 계산합니다.
   */
  ...
  /**
   * 최종 확인 모달에서 확인 버튼 클릭 처리
   *
   * 설명:
   * - 실제로 onSubmit을 호출하여 캠페인을 등록합니다.
   */
  const handleFinalConfirm = () => {
    // 모달 닫기
    setFinalConfirmModal({ is_open: false });
   ...
   *
   * 설명:
   * - 수정 모드이거나 필수 항목이 채워지지 않았으면 바로 onSubmit 호출
   * - 생성 모드이고 필수 항목이 모두 채워졌으면 최종 확인 모달 표시
   */
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();

    // 수정 모드이거나 필수 항목 검증이 통과하지 않으면 바로 제출
    ...
    // 생성 모드이고 필수 항목이 모두 채워졌으면 최종 확인 모달 표시
    setFinalConfirmModal({ is_open: true });
  };
  ...
  return (
    <>
      ...
      {/* 최종 확인 모달 (캠페인 등록 시) */}
      <BaseModal
        is_open={finalConfirmModal.is_open}
        on_close={() => setFinalConfirmModal({ is_open: false })}
        message="캠페인 진행 시에는 삭제/수정이 불가합니다.<br>캠페인을 등록하시겠습니까?"
        buttons={["취소", "확인"]}
        on_confirm={handleFinalConfirm}
      />
      ...
      <form onSubmit={handleSubmit} className={infoStyles.campaign_form}>
        ...
        <section className={styles.section}>
          ...
          {/* 모집 관련 필드 */}
          {/* 배송형에서는 홍보 링크 밑에 모집 인원 필드가 있으므로, 
