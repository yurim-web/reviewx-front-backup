...
export default function ReviewCampaignForm({
   ...
   *
   * 설명:
   * - 구매평 캠페인의 차감 포인트는 {(구매 지급 포인트 × 모집 인원) + (추가 지급 포인트 × 모집 인원)}입니다.
   */
  const deductedPoints = useMemo(() => {
    ...
    const purchasePoints =
      Number(String(formData.purchasePoints).replace(/,/g, "")) || 0;
    return (
      purchasePoints * recruitmentCount + additionalPoints * recruitmentCount
    );
  }, [
    formData.recruitmentCount,
        ...
        <section className={styles.section}>
          ...
          <article className={infoStyles.form_group}>
            ...
            <input
              ...
              placeholder="구매 플랫폼, 브랜드, 제공하는 서비스/제품 등"
          ...
          {/* 포인트 관리 섹션 (구매평 특수: purchasePoints 포함) */}
          ...
          {/* 구매평에서는 등록기간을 따로 표시하므로 RecruitmentFieldsSection에서는 등록기간을 표시하지 않습니다 */}
