 ...
 * 구분 필터 모달 컴포넌트 (공통)
 *
 * 목적: GA/SA 관리자 카테고리 목록 페이지에서 구분을 필터링하는 모달입니다.
 *
 * 사용 위치:
 * - CategoryFilterSection 컴포넌트에서 구분 필터로 사용
...
const division_options: { value: CategoryDivision; label: string }[] = [
...
const get_division_options = (): FilterOption<CategoryDivision>[] => {
...
export default function DivisionFilterModal({
  ...
  return (
    <BaseFilterModal<CategoryDivision>
      is_open={is_open}
      on_close={on_close}
