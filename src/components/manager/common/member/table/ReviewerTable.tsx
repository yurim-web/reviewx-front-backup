...
const ReviewerTable = forwardRef<ReviewerTableRef, ReviewerTableProps>(function ReviewerTable(
  {
    ...
    selected_types = [],
    selected_statuses = [],
    on_restriction_click: _on_restriction_click,
    styles: cssStyles,
    detail_path,
  },
  ref
  ...
  // 체크박스 선택 상태 관리 (공통 훅)
  const {
    selected_ids: selected_reviewer_ids,
    handle_checkbox_toggle,
    handle_select_all: toggle_all_selection,
    reset_selection,
  } = useTableSelection();
  ...
  const filtered_reviewers = useMemo(() => {
    ...
    return reviewers_to_filter.filter((reviewer) => {
      // 검색어 필터
      if (search_query) {
        const matches_search = reviewer.name.toLowerCase().includes(search_query.toLowerCase());
  ...
  const restriction = useRestrictionHandler({
    selectedIds: selected_reviewer_ids,
    findMember: (id) => {
      const r = filtered_reviewers.find((rev) => rev.id === id);
      return r ? { id: r.id, display_name: r.name, current_points: r.current_points } : undefined;
    },
    division: "리뷰어",
    updateStatusType: update_reviewer_status_type,
    reset_selection,
  });
  ...
  const is_all_selected =
    selected_reviewer_ids.length === filtered_reviewers.length && filtered_reviewers.length > 0;

  // 전체 선택/해제 핸들러
  const handle_select_all = () => {
    toggle_all_selection(
      filtered_reviewers.map((r) => r.id),
      is_all_selected
    );
  };
  ...
  return (
    <div
      className={`${cssStyles.table_container} ${tooltip_container_styles.tooltip_container_visible}`}
    >
              ...
              return (
                ...
                >
                  ...
                  <div className={cssStyles.table_cell_channel}>
                    <div className={cssStyles.channel_icons}>
                      {reviewer.channels.map((channel, index) => (
                        <div key={index} className={cssStyles.channel_icon_wrapper}>
                          <img
                  ...
                  <div className={cssStyles.table_cell_campaign_participated}>
                    <EllipsisTooltip
                      content={`${reviewer.campaign_participated.toLocaleString()}회`}
                    >
                      <span>{reviewer.campaign_participated.toLocaleString()}회</span>
                    </EllipsisTooltip>
                  </div>

                  {/* 캠페인 완료 */}
                  <div className={cssStyles.table_cell_campaign_completed}>
                    <EllipsisTooltip content={`${reviewer.campaign_completed.toLocaleString()}회`}>
                      <span>{reviewer.campaign_completed.toLocaleString()}회</span>
                    </EllipsisTooltip>
                  </div>

                  {/* 보유 포인트 */}
                  <div className={cssStyles.table_cell_current_points}>
                    <EllipsisTooltip content={reviewer.current_points.toLocaleString()}>
                      <span>{reviewer.current_points.toLocaleString()}</span>
                    </EllipsisTooltip>
                  </div>

                  {/* 출금 포인트 */}
                  <div className={cssStyles.table_cell_withdrawn_points}>
                    <EllipsisTooltip content={reviewer.withdrawn_points.toLocaleString()}>
                      <span>{reviewer.withdrawn_points.toLocaleString()}</span>
                    </EllipsisTooltip>
                  </div>
                  ...
                  <div className={cssStyles.table_cell_status}>
                    <MemberStatusTag
                      status={reviewer.status as "정상" | "일시 정지" | "영구 정지"}
                    />
                  </div>
                </div>
              );
