 ...
 */
export function usePieChartClickHandler(
  ...
  useEffect(() => {
    ...
    const handle_click = (e: MouseEvent) => {
      ...
      const paths = container.querySelectorAll<SVGPathElement>(finalOptions.selector);
      paths.forEach((path) => {
        // 흰색 선 유지
        path.setAttribute("stroke", finalOptions.stroke);
        path.style.setProperty("stroke", finalOptions.stroke, "important");
        path.style.setProperty("stroke-width", finalOptions.strokeWidth, "important");
        // outline 제거
  ...
  }, [containerRef, finalOptions.stroke, finalOptions.strokeWidth, finalOptions.selector]);
}
