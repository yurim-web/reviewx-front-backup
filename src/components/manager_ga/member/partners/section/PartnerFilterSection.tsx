 ...
 * 파트너 필터 섹션 컴포넌트
 *
 * 목적: 파트너 목록을 필터링하기 위한 필터 버튼들을 표시하는 섹션입니다.
 *
 * 사용 페이지:
...
import ChannelFilterModal from '@/components/manager_ga/member/partners/filter/ChannelFilterModal';
import DivisionFilterModal from '@/components/manager_ga/member/partners/filter/DivisionFilterModal';
import TypeFilterModal, {
  type PartnerType,
} from '@/components/manager_ga/member/partners/filter/TypeFilterModal';
import StatusFilterModal from '@/components/manager_ga/member/partners/filter/StatusFilterModal';
import type {
  Channel,
  PartnerDivision,
  PartnerStatus,
...
export default function PartnerFilterSection({
  ...
  // 구분 필터 핸들러
  const handle_division_apply = (divisions: PartnerDivision[]) => {
    set_selected_divisions(divisions);
  };

  const handle_remove_division = (division: PartnerDivision) => {
    set_selected_divisions(selected_divisions.filter((d) => d !== division));
  };

  // 유형 필터 핸들러
  const handle_type_apply = (types: PartnerType[]) => {
    set_selected_types(types);
  };

  const handle_remove_type = (type: PartnerType) => {
    set_selected_types(selected_types.filter((t) => t !== type));
  };

  // 상태 필터 핸들러
  const handle_status_apply = (statuses: PartnerStatus[]) => {
    set_selected_statuses(statuses);
  };

  const handle_remove_status = (status: PartnerStatus) => {
  ...
  const handle_filter_tag_remove = (value: string) => {
    ...
    } else if (selected_divisions.includes(value as PartnerDivision)) {
      handle_remove_division(value as PartnerDivision);
    } else if (selected_types.includes(value as PartnerType)) {
      handle_remove_type(value as PartnerType);
    } else if (selected_statuses.includes(value as PartnerStatus)) {
      handle_remove_status(value as PartnerStatus);
    }
  };
        ...
        filter_modal_button={
          <>
            ...
            <div className={styles.filter_item}>
              ...
              />
              <span className={styles.filter_text}>파트너 목록 다운로드</span>
            </div>
          </>
        }
        // 오른쪽 액션 버튼 (차단)
