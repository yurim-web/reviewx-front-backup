...
export default function ReportReasonModal({
  ...
  // 신고 사유 텍스트 상태 관리
  // 초기값은 props로 받은 report_reason 또는 code_info의 reason을 사용
  const [reason_text, set_reason_text] = useState<string>(
    report_reason || code_info?.reason || '신고 사유가 없습니다.',
  );

  // 모달이 열릴 때마다 신고 사유 텍스트 초기화
  ...
  }, [is_open, report_reason, code_info?.reason]);
  ...
  return (
    <div className={styles.modal_overlay} onClick={handle_overlay_click}>
      ...
      >
        ...
        {/* 신고 사유 텍스트 영역 (input 스타일) */}
        <div className={styles.report_reason_box}>
          <textarea
            className={styles.report_reason_text}
            value={reason_text}
