...
- **`TabNavigation`** - 상단 탭 네비게이션 (캠페인/포인트/계정/커뮤니티)

### 주요 기능:

- **프로필 섹션**: 프로필 이미지, 닉네임, 역할 표시
- **서브 탭**: 프로필/채널·스토어 전환
- **메뉴 항목들**:
  - 이용 가이드 (외부 링크 - 새창 열림)
  - 공지사항 (내부 페이지 이동)
  - 자주 묻는 질문 (내부 페이지 이동)
  - 카카오톡 상담 (외부 링크 - 새창 열림)
- **채널 관리**: 네이버 블로그, 인스타그램, 유튜브, 틱톡 연결
- **스토어 관리**: 네이버쇼핑, 쿠팡, 카카오쇼핑 등 8개 쇼핑몰 연결
...
**파일**: `src/app/user/delivery/[id]/page.tsx` (각 타입별로 분산)
